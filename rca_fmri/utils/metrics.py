...
def icc_full(subjects, values, version="ICC1"):
    '''
    Compute the intraclass correlation coefficient (ICC) for a set of subjects and values.
    Input:
...
def icc11(subjects, values, alpha=.05, r0=0, return_stats=True):
    """Compute ICC(1,1).
    ...
    data = np.asarray(np.split(np.asarray(values)[np.argsort(indices)], np.cumsum(np.bincount(indices))[:-1]))
    assert data.ndim == 2
    n,k = data.shape
...
def contrastive_loss(embedding, label, eps):
    ...
    dist = torch.cdist(embedding, embedding, p=2)  # gives matrix with (i,j) = l2 norm of (output[i:]-output[j:])
...
def info_nce(embedding, label, temp=0.1): 
    '''
    Computes the InfoNCE loss objective for two sets of embeddings using the L2 norm instead of cosine similarity since we use one-dimensional embeddings most of the time. 
    Input: 
    ...
    sim = -dist / temp  # [N, N]
    # sum distance over all positive pairs; if we minimise this then we maximise the similarity between positive pairs
    num = - torch.mean(torch.masked_select(sim, same))
    # sum distance over all pairs; if we minimise this then we maximise the similarity between all pairs
